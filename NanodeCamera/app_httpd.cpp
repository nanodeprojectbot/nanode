    ...
    if (align_face(net_boxes, image_matrix, aligned_face) == ESP_OK)
    {
        ...
        else
        {
            ...
            if (matched_id >= 0)
            {
                ...
                rgb_printf(image_matrix, FACE_COLOR_GREEN, "Face Recognized", matched_id);
            }
            else
            {
                Serial.println("No Match Found");
                rgb_print(image_matrix, FACE_COLOR_RED, "Face Found");
                matched_id = -1;
            }
        }
    }
...
static esp_err_t Test1_handler(httpd_req_t *req)
{
    ...
    if (buf_len > 1)
    {
        ...
        if (httpd_req_get_url_query_str(req, buf, buf_len) == ESP_OK)
        {
            if (httpd_query_key_value(buf, "var", 
            variable, sizeof(variable)) == ESP_OK)  
